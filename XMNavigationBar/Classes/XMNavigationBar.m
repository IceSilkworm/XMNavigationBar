...
static void xm_exchangeImplementations(Class class, SEL originalSelector, SEL swizzledSelector) {
...
- (UIColor *)xm_specifiedTextColor {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setXm_specifiedTextColor:(UIColor *)xm_specifiedTextColor {
    objc_setAssociatedObject(self, @selector(xm_specifiedTextColor), xm_specifiedTextColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        xm_exchangeImplementations(class, @selector(setAttributedText:), @selector(xm_setAttributedText:));
    });
}

- (void)xm_setAttributedText:(NSAttributedString *)attributedText {
    if (self.xm_specifiedTextColor) {
        ...
        [mutableAttributedText addAttributes:@{ NSForegroundColorAttributeName : self.xm_specifiedTextColor} range:NSMakeRange(0, mutableAttributedText.length)];
        attributedText = mutableAttributedText;
    }
    [self xm_setAttributedText:attributedText];
}

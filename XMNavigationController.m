...
BOOL shouldShowFake(UIViewController *vc, UIViewController *from, UIViewController *to) {
    ...
    if (from.xm_splitNavigationBarTransition || to.xm_splitNavigationBarTransition) {
        return YES;
    }
    
    if (from.xm_computedBarImage && to.xm_computedBarImage && isImageEqual(from.xm_computedBarImage, to.xm_computedBarImage)) {
        // have the same image
        return from.xm_barAlpha != to.xm_barAlpha;
    }
    
    if (!from.xm_computedBarImage && !to.xm_computedBarImage && [from.xm_computedBarTintColor.description isEqual:to.xm_computedBarTintColor.description]) {
...
void adjustLayout(UIViewController *vc) {
    BOOL isTranslucent = vc.xm_barHidden || vc.xm_barAlpha < 1.0;
    if (!isTranslucent) {
        UIImage *image = vc.xm_computedBarImage;
        if (image) {
            isTranslucent = imageHasAlphaChannel(image);
        } else {
            UIColor *color = vc.xm_computedBarTintColor;
...
@interface XMNavigationControllerDelegate : UIScreenEdgePanGestureRecognizer <UINavigationControllerDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, weak) id<UINavigationControllerDelegate> proxiedDelegate;
@property (nonatomic, weak, readonly) XMNavigationController *nav;

- (instancetype)initWithNavigationController:(XMNavigationController *)navigationController;

@end

@interface XMNavigationController ()

@property (nonatomic, readonly) XMNavigationBar *navigationBar;
...
@property (nonatomic, strong) XMNavigationControllerDelegate *navigationDelegate;
...
@implementation XMNavigationControllerDelegate

- (instancetype)initWithNavigationController:(XMNavigationController *)nav {
    if (self = [super init]) {
        _nav = nav;
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (self.nav.viewControllers.count > 1) {
        // 先判断xm_swipeBackEnabled再判断xm_backInteractive，
        // 可以解决当用户已经将xm_swipeBackEnabled设置为NO时，并且重写了xm_backInteractive的getter方法，用手势返回时，先调用xm_backInteractive的getter方法的问题
        // 应该是已经将xm_swipeBackEnabled设置为NO后，用户再进行侧滑手势时，不触发任何操作。
        return self.nav.topViewController.xm_swipeBackEnabled && self.nav.topViewController.xm_backInteractive;
    }
    return NO;
}
...
- (void)handleNavigationTransition:(UIScreenEdgePanGestureRecognizer *)pan {
    XMNavigationController *nav = self.nav;
    if (![self.proxiedDelegate respondsToSelector:@selector(navigationController:interactionControllerForAnimationController:)]) {
        id<XMNavigationTransitionProtocol> target = (id<XMNavigationTransitionProtocol>)[nav superInteractivePopGestureRecognizer].delegate;
    ...
    if (coordinator) {
        ...
        if (pan.state == UIGestureRecognizerStateBegan || pan.state == UIGestureRecognizerStateChanged) {
            nav.navigationBar.tintColor = blendColor(from.xm_tintColor, to.xm_tintColor, coordinator.percentComplete);
        }
    }
}
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (!viewController.xm_extendedLayoutDidSet) {
        adjustLayout(viewController);
        viewController.xm_extendedLayoutDidSet = YES;
    }
...
- (void)showViewController:(UIViewController * _Nonnull)viewController withCoordinator: (id<UIViewControllerTransitionCoordinator>)coordinator {
    ...
    if (@available(iOS 12.0, *)) {
        // Fix a system bug https://github.com/listenzz/XMNavigationBar/issues/35
    ...
    if (self.nav.poppingViewController) {
        ...
        if (backButtonLabel) {
            backButtonLabel.xm_specifiedTextColor = backButtonLabel.textColor;
        }
        
        [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
            
        } completion:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
            backButtonLabel.xm_specifiedTextColor = nil;
        }];
    }
        ...
        if (shouldFake) {
            ...
            if (@available(iOS 13.0, *)) {
                if (to == viewController) {
                    self.nav.navigationBar.scrollEdgeAppearance.backgroundColor = viewController.xm_computedBarTintColor;
                    self.nav.navigationBar.scrollEdgeAppearance.backgroundImage = viewController.xm_computedBarImage;
                    self.nav.navigationBar.standardAppearance.backgroundColor = viewController.xm_computedBarTintColor;
                    self.nav.navigationBar.standardAppearance.backgroundImage = viewController.xm_computedBarImage;
                }
            }
        }
    } completion:^(id<UIViewControllerTransitionCoordinatorContext> _Nonnull context) {
...
- (instancetype)initWithRootViewController:(UIViewController *)rootViewController {
    if (self = [super initWithNavigationBarClass:[XMNavigationBar class] toolbarClass:nil]) {
...
- (instancetype)initWithNavigationBarClass:(Class)navigationBarClass toolbarClass:(Class)toolbarClass {
    NSAssert([navigationBarClass isSubclassOfClass:[XMNavigationBar class]], @"navigationBarClass Must be a subclass of XMNavigationBar");
    return [super initWithNavigationBarClass:navigationBarClass toolbarClass:toolbarClass];
}

- (instancetype)init {
    return [super initWithNavigationBarClass:[XMNavigationBar class] toolbarClass:nil];
}

- (void)setDelegate:(id<UINavigationControllerDelegate>)delegate {
    if ([delegate isKindOfClass:[XMNavigationControllerDelegate class]] || !self.navigationDelegate) {
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    return self.topViewController.xm_barStyle == UIBarStyleBlack ? UIStatusBarStyleLightContent : UIStatusBarStyleDefault;
}
...
- (void)viewDidLoad {
    ...
    self.navigationDelegate = [[XMNavigationControllerDelegate alloc] initWithNavigationController:self];
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item {
    if (self.viewControllers.count > 1 && self.topViewController.navigationItem == item ) {
        if (!(self.topViewController.xm_backInteractive && self.topViewController.xm_clickBackEnabled)) {
...
- (void)fixClickBackIssue {
    ...
    if (@available(iOS 11.0, *)){
        ...
        if (!(coordinator && coordinator.interactive)) {
            self.navigationBar.barStyle = self.topViewController.xm_barStyle;
            self.navigationBar.titleTextAttributes = self.topViewController.xm_titleTextAttributes;
        }
    }
}
...
- (void)updateNavigationBarStyleForViewController:(UIViewController *)vc {
    self.navigationBar.barStyle = vc.xm_barStyle;
}

- (void)updateNavigationBarTinitColorForViewController:(UIViewController *)vc {
    self.navigationBar.tintColor = vc.xm_tintColor;
    self.navigationBar.titleTextAttributes = vc.xm_titleTextAttributes;
    if (@available(iOS 13.0, *)) {
        self.navigationBar.scrollEdgeAppearance.titleTextAttributes = vc.xm_titleTextAttributes;
        self.navigationBar.standardAppearance.titleTextAttributes = vc.xm_titleTextAttributes;
    }
}

- (void)updateNavigationBarAlphaForViewController:(UIViewController *)vc {
    if (vc.xm_computedBarImage) {
        self.navigationBar.fakeView.alpha = 0;
        self.navigationBar.backgroundImageView.alpha = vc.xm_barAlpha;
    } else {
        self.navigationBar.fakeView.alpha = vc.xm_barAlpha;
        self.navigationBar.backgroundImageView.alpha = 0;
    }
    
    if (vc.xm_barAlpha == 0) {
        self.navigationBar.xm_backgroundView.layer.mask = [CALayer new];
    } else {
        self.navigationBar.xm_backgroundView.layer.mask = nil;
    }
    
    self.navigationBar.shadowImageView.alpha = vc.xm_computedBarShadowAlpha;
}

- (void)updateNavigationBarBackgroundForViewController:(UIViewController *)vc {
    self.navigationBar.barTintColor = vc.xm_computedBarTintColor;
    self.navigationBar.backgroundImageView.image = vc.xm_computedBarImage;
}
...
- (void)showFakeBarFrom:(UIViewController *)from {
    self.fromFakeImageView.image = from.xm_computedBarImage;
    self.fromFakeImageView.alpha = from.xm_barAlpha;
    self.fromFakeImageView.frame = [self fakeBarFrameForViewController:from];
    [from.view addSubview:self.fromFakeImageView];
    
    self.fromFakeBar.subviews.lastObject.backgroundColor = from.xm_computedBarTintColor;
    self.fromFakeBar.alpha = from.xm_computedBarImage ? 0 : from.xm_barAlpha;

    if (from.xm_barAlpha == 0 || from.xm_computedBarImage) {
    ...
    self.fromFakeShadow.alpha = from.xm_computedBarShadowAlpha;
...
- (void)showFakeBarTo:(UIViewController * _Nonnull)to {
    self.toFakeImageView.image = to.xm_computedBarImage;
    self.toFakeImageView.alpha = to.xm_barAlpha;
    self.toFakeImageView.frame = [self fakeBarFrameForViewController:to];
    [to.view addSubview:self.toFakeImageView];
    
    self.toFakeBar.subviews.lastObject.backgroundColor = to.xm_computedBarTintColor;
    self.toFakeBar.alpha = to.xm_computedBarImage ? 0 : to.xm_barAlpha;
    self.toFakeBar.frame = [self fakeBarFrameForViewController:to];
    [to.view addSubview:self.toFakeBar];
    
    self.toFakeShadow.alpha = to.xm_computedBarShadowAlpha;
